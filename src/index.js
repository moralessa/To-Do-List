...
import {
  pinExpanded, expandNav, unPinExpanded, hoverPopUp, destroyPopUp, populateNavProjects,
  removeNavProject,
} from './modules/nav.js';
...
import {
  addTaskCountToLocale, addTaskToLocale, addProjectToLocale, getTaskCount,
  getProjectCount, getTasks, getProjects, addTaskToProjectLocale,
  removeProjectFromLocale, removeTaskFromLocale, removeTaskFromProjectLocale,
} from './modules/localeStore';
...
const faviconContainer = document.createElement('link');
...
/* IFEE to fill inbox with dummy data and add event listener for inbox nav
list items if locale storage is empty */
  (function defaultLoad() {
    const date = new Date();
    const dateFormattedUs = date.toLocaleDateString('en-US');
    const tasksItems = ['Take out the trash', 'Feed the fishes', 'Use Suru'];
    const newProject = createNewProject('Inbox', projectsArr);
    for (let i = 0; i < tasksItems.length; i += 1) {
      const newTask = createNewTask(i, tasksItems[i], dateFormattedUs, tasksArr);
...
(function dateInputInitialize() { // IIFE to set min date of input to the current date
  const date = new Date();
...
inbox.addEventListener('mouseenter', () => { hoverPopUp(inbox); });
inbox.addEventListener('mouseleave', destroyPopUp);
add.addEventListener('mouseenter', () => { hoverPopUp(add); });
...
function checkIfMainIsPopulized(target) {
  const temp = document.querySelector('.title');
...
function projectAddition(name, task) {
  ...
  if (ndx === undefined) {
    ...
    projectCount += 1;
    const newProject = createNewProject(name, projectsArr);
...
function taskAddition() {
  ...
  const newTask = createNewTask(taskCount, description, dateFormatted, tasksArr);
  // Add task to locale storage
  addTaskToLocale(newTask);
